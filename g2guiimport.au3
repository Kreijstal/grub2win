...
Func ImportRunGUI ()
	...
	If Ubound ($importarray) = 0 Or $rglinfind = "" Then
...
Func ImportParseSearch ($psrecord, ByRef $pslinarg)
	...
	Select
		...
		Case StringInStr ($psrecord, "search.fs_uuid")  Or StringInStr ($psrecord, " --fs-uuid ") Or StringInStr ($psrecord, " -u ")
			$psfind   = "UUID="  & $psfind  & " "
		Case StringInStr ($psrecord, "search.fs_label") Or StringInStr ($psrecord, " --label ")   Or StringInStr ($psrecord, " -l ")
			$psfind   = "LABEL=" & $psfind  & " "
		Case Else
			$psfind = ""
	EndSelect
...
Func ImportGetFile ()
	...
	If $importtype = "Linux"  Then $importfilepath = FileOpenDialog ("Select a Linux Config file to import", "C:\", _
		"Linux Config Files (*.cfg;*.conf)", $FD_FILEMUSTEXIST, "linux.grub.cfg")
	If @error Then Return ""
